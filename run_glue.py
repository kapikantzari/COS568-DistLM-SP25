...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            ##################################################
            # Aggregate gradients, averaging, then scatter
            ##################################################
            for param in model.parameters():
                if param.requires_grad:
                    gather_list = [torch.zeros_like(param.grad, device=args.device) for _ in range(args.world_size)]
                    torch.distributed.gather(param.grad, gather_list=gather_list if args.local_rank == 0 else None, dst=0)
                    if args.local_rank == 0:
                        gather_list = torch.mean(torch.stack(gather_list), dim=0)
                        gather_list = [gather_list for _ in range(args.world_size)]
                    torch.distributed.scatter(param.grad, gather_list if args.local_rank == 0 else None, src=0)

            tr_loss += loss.item()
            logging_loss = loss.item()
            all_loss.append(logging_loss)
    ...
    logger.info("Time: ", all_time)
    logger.info(f"Average time (excluding 1st iteration): {sum(all_time[1:])/(len(all_time)-1)}")
    logger.info(f"All losses: {all_loss}")
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                logger.info(f"loss: {tmp_eval_loss.mean().item()}")
...
def main():
    ...
    parser.add_argument("--master_ip", type=str, default='10.10.1.2', 
                        help="IP address for distributed training")
    parser.add_argument("--master_port", type=str, default='12345', 
                        help="port number for distributed training")
    parser.add_argument("--world_size", type=int, default=4, 
                        help="Number of nodes (4 in our experiments)")
    ...
    # Setup logging
    logging.basicConfig(filename=f'task2a_{args.local_rank}.log', 
                        filemode='a', 
                        format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                        datefmt = '%m/%d/%Y %H:%M:%S',
                        level = logging.INFO if args.local_rank in [-1, 0, 1, 2, 3] else logging.WARN)
